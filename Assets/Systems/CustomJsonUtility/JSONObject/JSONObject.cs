...
//			return Mathf.RoundToInt(n);
						...
						if((str[offset] == ',' && depth == 0) || depth < 0) {
							...
							if(inner.Length > 0) {
								...
								if(maxDepth != -1){ //maxDepth of -1 is the end of the line
									JSONObject _jsonToAdd = Create(inner, (maxDepth < -1) ? -2 : maxDepth - 1) ;
									_jsonToAdd.parent = this ;
									list.Add(_jsonToAdd);
								} else if(storeExcessLevels) {
									JSONObject _jsonToAdd = CreateBakedObject(inner) ;
									_jsonToAdd.parent = this ;
									list.Add(_jsonToAdd);
								}
	...
	// Added by Zafer
	// modified Add, AddField and Parse functions for parent define
	private JSONObject m_parent ;
...
#pragma warning restore 219
	...
	public JSONObject AddValue(string _key, float _value) {
		if (this.HasField(_key) && this[_key].IsNumber) {
			this[_key].n += _value ;
//			SetField(_key, this[_key].f + _value ) ;
		} else {
			SetField(_key, _value ) ;
		}
		return this ;		
	}	

	public bool isTrue(string _key) {
		return (this.HasField(_key) && this[_key].IsBool && this[_key].b) ;
	}
	public bool isTrue(string _key, float _value) {
		return (this.HasField(_key) && this[_key].IsNumber && this[_key].f == _value) ;
	}
	public bool isTrue(string _key, string _value) {
		return (this.HasField(_key) && this[_key].IsString && this[_key].str == _value) ;
	}

	public Vector3 GetVector3(string _keyX, string _keyY, string _keyZ) {
		Vector3 _result = new Vector3() ;
		if (this.HasField(_keyX)) _result.x = this[_keyX].f ;
		if (this.HasField(_keyY)) _result.y = this[_keyY].f ;
		if (this.HasField(_keyZ)) _result.z = this[_keyZ].f ;
		return _result ;		
	}		
	public JSONObject SetHasField(string _keyX, string _keyY, string _keyZ, ref Vector3 _refValue) {
		if (this.HasField(_keyX)) _refValue.x = this[_keyX].f ;
		if (this.HasField(_keyY)) _refValue.y = this[_keyY].f ;
		if (this.HasField(_keyZ)) _refValue.z = this[_keyZ].f ;
		return this ;		
	}		
	public JSONObject SetHasField(string _key, ref bool _refValue) {
		if (this.HasField(_key)) {
			_refValue = this[_key].b ;
		}
		return this ;		
	}	
	public JSONObject SetHasField(string _key, ref int _refValue) {
		if (this.HasField(_key)) {
			_refValue = this[_key].i ;
		}
		return this ;		
	}	
	public JSONObject SetHasField(string _key, ref float _refValue) {
		if (this.HasField(_key)) {
			_refValue = this[_key].f ;
		}
		return this ;		
	}	
	public JSONObject SetHasField(string _key, ref string _refValue) {
		if (this.HasField(_key)) {
			_refValue = this[_key].str ;
		}
		return this ;		
	}	
	public JSONObject SetHasField(string _key, ref JSONObject _refValue) {
		if (this.HasField(_key)) {
			_refValue = this[_key] ;
		}
		return this ;		
	...
	public JSONObject SetGetField(string _key) {
		if (!this.HasField(_key)) {
			this.AddField(_key, new JSONObject(JSONObject.Type.OBJECT)) ;
		}
		return this[_key] ;		
	}	
	public JSONObject SetGetField(string _key, string _defaultValue) {
		if (!this.HasField(_key)) {
			this.AddField(_key, _defaultValue) ;
		}
		return this[_key] ;		
	}
	public JSONObject SetGetField(string _key, int _defaultValue) {
		if (!this.HasField(_key)) {
			this.AddField(_key, _defaultValue) ;
		}
		return this[_key] ;		
	}
	public JSONObject SetGetField(string _key, float _defaultValue) {
		if (!this.HasField(_key)) {
			this.AddField(_key, _defaultValue) ;
		}
		return this[_key] ;		
	}
	public JSONObject SetGetField(string _key, bool _defaultValue) {
		if (!this.HasField(_key)) {
			this.AddField(_key, _defaultValue) ;
		}
		return this[_key] ;		
	}	
    public string getS(string _var) {
        JSONObject _json = getJsonValue(_var) ;
	...
	public float rand() {		
		float _min = 0f, _max = 1f;
		if (this.HasField("min")) {
			_min = this["min"].f ;
		}
		if (this.HasField("max")) {
			_max = this["max"].f ;
		}
    ...
    public JSONObject getJsonValue(string _var) {
        string[] _splitParam = _var.Replace(" ", "").Split('.') ;
        JSONObject _tempJson = this ; 
        for(int i = 0 ; i < _splitParam.Length ; i++) {
            if (_tempJson.HasField(_splitParam[i])) {
                _tempJson = _tempJson[_splitParam[i]] ; 
            } else {
                return null ;
            }
        }
...
//    public bool hasVar(string _var) {
//        return getJsonValue(_var) != null ;
//    }
       
    public void setValue(string _var, object _value) {
        string[] _splitParam = _var.Replace(" ", "").Split('.') ;
        if (_splitParam.Length == 1) {
            if (_value is string)
                this.SetField(_splitParam[0], (string)_value) ;
            else if (_value is int)
                this.SetField(_splitParam[0], (int)_value) ;
            else if (_value is float)
                this.SetField(_splitParam[0], (float)_value) ;
            else if (_value is bool)
                this.SetField(_splitParam[0], (bool)_value) ;
        } else {
            JSONObject _tempJson = this ; 
            for(int i = 0 ; i < _splitParam.Length - 1 ; i++) {
                if (_tempJson.HasField(_splitParam[i]) == false) {
                    _tempJson.AddField(_splitParam[i], new JSONObject(JSONObject.Type.OBJECT)) ;
                }
                _tempJson = _tempJson[_splitParam[i]] ;
            }
            if (_value is string)
                _tempJson.SetField(_splitParam[_splitParam.Length -1], (string)_value) ;
            else if (_value is int)
                _tempJson.SetField(_splitParam[_splitParam.Length -1], (int)_value) ;
            else if (_value is float)
                _tempJson.SetField(_splitParam[_splitParam.Length -1], (float)_value) ;
            else if (_value is bool)
                _tempJson.SetField(_splitParam[_splitParam.Length -1], (bool)_value) ;
        }
    }     
	public void InsertField(string name, int _index, JSONObject obj) {
		if(obj) {
			keys.Insert(_index, name);
			list.Insert(_index, obj);
		}
	}
	public void Insert(int _index, JSONObject obj) {
		if(obj) {
			list.Insert(_index, obj);
		}		
	}	
	public JSONObject FindAtList(string _key, string _search) {
		if (IsArray) {
			for (int i = 0; i < list.Count ;i++ ) {
				if (list[i].isTrue(_key, _search)){
					return list[i] ;
				}
			}
		}
	...
	public void sort(bool _ascending = false) {
		List<string> _keys = new List<string>(keys) ;
		if (IsObject) {
			_keys.Sort(delegate(string v1, string v2) {
				float j1_val = _ascending ? this[v1].f  : this[v2].f  ;
				float j2_val = _ascending ? this[v2].f  : this[v1].f  ;
				return j1_val.CompareTo(j2_val) ; 
			}) ;
		}		
		if (IsObject || IsArray) {
			list.Sort(delegate(JSONObject v1, JSONObject v2) {
				float j1_val = _ascending ? v1.f : v2.f ;
				float j2_val = _ascending ? v2.f : v1.f ;
				return j1_val.CompareTo(j2_val) ; 
			}) ;
		}
		if (IsObject) {
	...
	public bool containsAtList(int _search) {
		if (IsArray) {
			for (int i = 0; i < list.Count ;i++ ) {
				if (list[i].IsNumber && list[i].i == _search){
					return true ;
				}
			}
		}		
	...
	public bool containsAtList(string _search) {
		if (IsArray) {
			for (int i = 0; i < list.Count ;i++ ) {
				if (list[i].IsString && list[i].str == _search){
					return true ;
				}
			}
		}		
	...
	public bool containsAtList(List<string> _search) {
		if (IsArray) {
			for (int i = 0; i < list.Count ;i++ ) {
				bool _result = false ;
				for (int k = 0; k < _search.Count ;k++ ) {
					if (list[i].IsString && list[i].str == _search[k]){
						_result = true ;
						break ;
					}
				}
