...
namespace Sabresaurus.PlayerPrefsEditor
{
    public class PlayerPrefsEditor : EditorWindow
    {
        ...
        [MenuItem("Brew/PlayerPrefs Extended Editor")]
...
//                showEditorPrefs = (newIndex == 1);
 //           }
        ...
        private void DrawMainList()
        {
            ...
            int visibleCount = Mathf.FloorToInt(Screen.height / rowHeight);
                ...
                else if (valueType == typeof(string)) // if we're dealing with a string
                {
                    ...
                    else
                    {
                        ...
                        if (newValue.Length > 1 && newValue[0] == '{') {
                            ...
                            {
//                                ImportPlayerPrefsWizard _wizard = ScriptableWizard.DisplayWizard<JSONEditor>("Import PlayerPrefs", "Import");
                                JSONObject _json = JSONObject.Create(newValue) ;
                                
                                JSONEditor window = (JSONEditor)GetWindow(typeof(JSONEditor));
                                window.playerPrefKey = fullKey ;
                                window.JSON = _json.ToString(true) ;
                                window.Show();
                            }
                        } else {
...
//            automaticDecryption = EditorGUILayout.Toggle("Auto-Decryption", automaticDecryption);
            ...
            float buttonWidth = (EditorGUIUtility.currentViewWidth - 10) / 3f;

            if (showEditorPrefs == false)
            {
                if (GUILayout.Button("Save to File",GUILayout.Width(buttonWidth)))
                {
                    JSONObject _jsonToSave = JSONObject.obj ;
                    PlayerPrefPair[] _pairs = RetrieveSavedPrefs(PlayerSettings.companyName, PlayerSettings.productName) ;
                    for (int i = 0; i < _pairs.Length; i++)
                    {
                        if (_pairs[i].Key.StartsWith("unity.") || _pairs[i].Key == "UnityGraphicsQuality") {
                            continue ;
                        }
                        if (_pairs[i].Value is int) {
                            _jsonToSave.AddField(_pairs[i].Key, (int)_pairs[i].Value) ;                        
                        } else if (_pairs[i].Value is float) {
                            _jsonToSave.AddField(_pairs[i].Key, (float)_pairs[i].Value) ;                        
                            _jsonToSave.AddField(_pairs[i].Key + "_isFloat", 1) ;                        
                        } else {
                            _jsonToSave.AddField(_pairs[i].Key, JSONObject.Create(PlayerPrefs.GetString(_pairs[i].Key))) ;                        
                        }
                    }
                    var path = EditorUtility.SaveFilePanel(
                                "Save Player Prefs as JSON",
                                "",
                                "playerprefs" + ".json",
                                "json");
                    ...
                    if (path.Length != 0)
                    {
                        var jsonData = _jsonToSave.ToString() ;
                        if (jsonData != null)
            ...
            if (showEditorPrefs == false)
            {
                ...
                {
                    string path = EditorUtility.OpenFilePanel("Overwrite Player Prefs", "", "json");
                    if (path.Length != 0)
                    {
                        ...
                        if (_loadedJson != null) 
                        {
                            ...
                            for (int i = 0; i < _pairs.Length; i++)
                            {
                                ...
                                PlayerPrefs.DeleteKey(_pairs[i].Key) ;
                            }
            ...
            EditorGUILayout.BeginHorizontal();
            buttonWidth = (EditorGUIUtility.currentViewWidth - 10) / 2f;
